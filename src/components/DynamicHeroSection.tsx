...
const DynamicHeroSection = () => {
  ...
  // Préparer les données pour useTypewriter (toujours appelé)
  const currentSlideData = heroSection?.active_slides?.[currentSlide];
  const slideTitle = currentSlideData?.title || 'Bienvenue sur GidsBlue Advisory';
  const { displayedText: currentTitle, isComplete } = useTypewriter(slideTitle, 60);
  ...
  // Charger les données de la section hero
  useEffect(() => {
    if (dataFetched) return; // Éviter les requêtes multiples
    ...
    const fetchHeroSection = async () => {
      try {
        ...
        setError('Erreur lors du chargement de la section hero');
  ...
  useEffect(() => {
    if (!heroSection?.active_slides?.length) return;

    const currentSlideDuration = heroSection.active_slides[currentSlide]?.slide_duration || 6000;
    
    const timer = setInterval(() => {
      setCurrentSlide((prev) => (prev + 1) % heroSection.active_slides.length);
    }, currentSlideDuration);

    return () => clearInterval(timer);
  }, [heroSection?.active_slides?.length]); // Supprimé currentSlide des dépendances

  // Si pas de section hero active, afficher un message ou une section par défaut
  if (loading) {
    return (
      <section className="relative min-h-screen w-full flex items-center bg-gradient-to-br from-blue-600 via-blue-500 to-blue-800 text-white">
  ...
  if (error || !heroSection || !heroSection.active_slides?.length) {
    return (
      <section className="relative min-h-screen w-full flex items-center bg-gradient-to-br from-blue-600 via-blue-500 to-blue-800 text-white">
        <div className="container mx-auto px-4 py-20 text-center">
          ...
          <p className="text-xl md:text-2xl text-blue-100 mb-8">
            Excellence en Audit Interne et Formation CIA
          </p>
            ...
            <div className="space-y-6">
              ...
              <h2 className="text-xl md:text-2xl lg:text-3xl font-light text-blue-100/90 leading-relaxed animate-slideInUp">
                <span className="bg-gradient-to-r from-blue-200 to-cyan-200 bg-clip-text text-transparent">
                  {currentSlideData?.subtitle || 'Excellence en Audit Interne et Formation CIA'}
                </span>
              </h2>
              
              <p className="text-lg md:text-xl text-blue-50/90 leading-relaxed max-w-2xl animate-slideInUp" style={{ animationDelay: '0.2s' }}>
                {currentSlideData?.description || 'Transformez vos processus d\'audit avec notre expertise de plus de 12 ans. Solutions sur mesure pour une gouvernance d\'entreprise optimale.'}
              </p>
            </div>
